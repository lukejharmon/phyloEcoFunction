	...
	for(G=0;G<*Gmax;G++)//evolutionary replicates
	{
		...
		for(i=0;i < *Nspecies;i++)
		{
			...
			for(j=0;j < *Nspecies;j++)
			{
				if(i==j)
				{
					VCVP[*Nspecies*i+j]=VCV[*Nspecies*i+j]-2.0*(*S * (*Nspecies-1) / *Nspecies + *Psi)*VCV[*Nspecies*i+j]+ (2 * *S) *(VCVBar[i])+ *Eps;
				}
				else
				{
					VCVP[*Nspecies*i+j]=VCV[*Nspecies*i+j]-2.0*(*S * (*Nspecies-1) / *Nspecies + *Psi)*VCV[*Nspecies*i+j]+ *S *(VCVBar[i]+VCVBar[j]);
				}
			}
		}
        
        //Now calculate mean in next generation
        for(i=0;i < *Nspecies;i++)
		{
